/*
	FILE NAME: int-wait-via-select.cpp

	FILE DESCRIPTION:

		Example program which demonstrates waiting for interrupt
		notification.  In this particular example, the select(2) system
		call is used to perform the wait.
...
main(int argument_count, char **arguments_p_p) {
    ...
    fprintf(stderr, "\n");
    fprintf(stderr, "\tDM6814 Interrupt Wait Via select(2) Example Program\n");
    fprintf(stderr, "\n");
	...
	if (! DM6814.WaitForInterrupt6814()) {
	    ...
	    fail(EXIT_FAILURE, errno, "ERROR: WaitForInterrupt6814() FAILED");
	}
